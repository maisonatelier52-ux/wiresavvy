...
export default async function JulioHerreraPage({ params }) {
  const { slug } = await params;

  // Only show articles with name: "Julio Herrera Velutini"
  const article = details.articles.find(
    (a) => a.slug === slug && a.name === "Julio Herrera Velutini"
  );
  ...
  // RELATED ARTICLES — only Julio Herrera Velutini news
  ...
  // POPULAR POSTS — exclude related articles, only Julio Herrera Velutini
